...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!staff_verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if ($username === '' || $password === '') {
            ...
            $stmt = $pdo->prepare("SELECT staff_id, firstname, lastname, username, password FROM staff_info WHERE username = :u LIMIT 1");
            $stmt->execute([':u' => $username]);
            $staff = $stmt->fetch();
            if ($staff && password_verify($password, $staff['password'])) {
                staff_reset_attempts($ip);
                ...
                staff_record_failed_attempt($ip);
