...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['fp_step1'])) {
        ...
        if (!is_safe_input($username) || empty($username)) {
            ...
            if (!$found_id) {
                ...
                $s = $pdo->prepare("SELECT sa.question_id FROM security_answer sa WHERE sa.user_id = ? AND sa.user_type = ?");
                ...
                if (!$sq) {
                    ...
                    $_SESSION['fp_question_id'] = $sq['question_id'];
                    $step = 2;
                }
            }
        }
    ...
    elseif (isset($_POST['fp_step2'])) {
        ...
        $submitted_question_id = (int)($_POST['question_id'] ?? 0);
        $answer                = trim($_POST['sq_answer'] ?? '');

        if ($submitted_question_id <= 0) {
            $error               = 'Please select a security question.';
            $step                = 2;
            $_SESSION['fp_step'] = 2;
        } elseif (empty($answer) || !is_safe_input($answer, 'alnumspace')) {
            $error               = 'Invalid answer. Only letters, numbers and spaces allowed.';
            $step                = 2;
            $_SESSION['fp_step'] = 2;
        } elseif ($submitted_question_id !== (int)$_SESSION['fp_question_id']) {
...
<div class="card border-0 shadow-sm" style="width:100%;max-width:420px;">
    <div class="card-body p-5">
        ...
        <p class="small text-muted mb-3">
            Select your security question and enter your answer.
        </p>
        <form method="POST" autocomplete="off" novalidate id="step2-form">
            <div class="mb-3">
                <label class="form-label small fw-semibold text-secondary">Security Question</label>
                <select name="question_id" class="form-control" id="fp_question_select" required>
                    <option value="">— Select your security question —</option>
                    <?php foreach ($all_questions as $q): ?>
                    <option value="<?= $q['id'] ?>"><?= htmlspecialchars($q['question']) ?></option>
                    <?php endforeach; ?>
                </select>
                <div class="invalid-feedback-custom" id="hint-question">Please select a question.</div>
            </div>
            <div class="mb-4">
                <label class="form-label small fw-semibold text-secondary">Your Answer</label>
                <div class="input-group">
                    <span class="input-group-text bg-white border-end-0">
                        <i class="fas fa-shield-halved text-secondary small"></i>
                    </span>
                ...
                <div style="font-size:11px;color:#6c757d;margin-top:5px;">
                    <i class="fas fa-info-circle me-1"></i>Answer is case-insensitive.
                </div>
            </div>
        ...
        <form method="POST" autocomplete="off" novalidate id="step3-form">
            <div class="mb-3">
                ...
                <div class="input-group">
                    <span class="input-group-text bg-white border-end-0">
                        <i class="fas fa-lock text-secondary small"></i>
                    </span>
...
(function () {
    const questionSelect = document.getElementById('fp_question_select');
    ...
    function checkStep2() {
        if (!step2Btn) return;
        const qValid = questionSelect && questionSelect.value !== '';
        const aVal   = answerInput ? answerInput.value : '';
        const aValid = spaceRegex.test(aVal) && aVal.trim().length > 0;
        step2Btn.disabled = !(qValid && aValid);
    }
    ...
    if (questionSelect) {
        questionSelect.addEventListener('change', checkStep2);
    }
